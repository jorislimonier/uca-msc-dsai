...
def safe_extract(tar: tarfile.TarFile, path, members=None, *, numeric_owner=False):
  print("Entered safe_extract")
  ...
  for member in tqdm(iterable=tar.getmembers(), total=len(tar.getmembers())):
    ...
    tar.extract(path=path, member=member, numeric_owner=numeric_owner)
...
class PreprocessH36m:
  ...
  def __init__(
    self, metadata, original_dir, extracted_dir=None, processed_dir=None, sample_rate=5
  ):
  ...
  def extract_tgz(self):
    ...
    for subject in self.subjects_annot:
      ...
      for file in self.extract_files:
        filename = join(cur_dir, f"{file}_{subject}.tgz")
      ...
      for subject in self.subjects_splits[data_split]:
        for action, subaction in self.sequence_mappings[subject].keys():
          ...
          for camera in self.camera_ids:
            imgnames, centers, scales, kps2d, kps3d = self._load_annotations(
  ...
  def _load_annotations(self, subject, action, subaction, camera):
    ...
    kps_3d = (
      kps_3d.reshape((num_frames, 32, 3))[:: self.sample_rate, self.movable_joints]
      / 1000.0
    )
