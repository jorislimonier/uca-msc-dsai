...
def experiment_not_implemented_message(experiment_name):
  error = f"{experiment_name} is not available! " f"Possible are: 'mnist'."
...
def get_trainer(experiment_name, device, optimizer_name, lr, seed):
  ...
  model = get_model(experiment_name=experiment_name, device=device)
...
def get_aggregator(
  ...
  if aggregator_type == "centralized":
    ...
    error_message = (
      f"{aggregator_type} is not a possible aggregator type, possible are: "
    )
