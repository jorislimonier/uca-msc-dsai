...
# package inheriting most of matplotlib package functions with shorter syntax
import pylab as pl
...
VAL = pd.read_csv(
    "http://www.i3s.unice.fr/~riveill/dataset/Amazon_Unlocked_Mobile/val.csv.gz")
TEST = pd.read_csv(
    "http://www.i3s.unice.fr/~riveill/dataset/Amazon_Unlocked_Mobile/test.csv.gz")

# pass an integer value to specify the number of header lines to output.
TRAIN.head(10)
...
# processed as a list for future analysis
unique_words = list(set(flatten_reviews))
# set allows to get unique words contain in flatten_reviews
...
pl.xlabel('word occurrence')
pl.ylabel('probability density')
pl.title('word occurences distribution')
pl.show()
...
pl.xlabel('sentence length')
pl.ylabel('density')
pl.title('sentence lengths distribution')
...
if run_failure:  # This code will give you an error
    reviews_tokenized_val = [word_tokenize(
        review) for review in VAL['Reviews']]
    for sentence in reviews_tokenized_val[:10]:
        print('%s \n' % sentence)
    ...
    for review in VAL['Reviews']:
        try:  # Store the result of the function word_tokenize applied on current review if it does not fail
            ...
            continue
    print('How many fails over the whole Validation reviews ? ',
          len(VAL['Reviews']) - len(reviews_tokenized_val))
...
flatten_reviews_val = [
    item for sublist in reviews_tokenized_val for item in sublist]
unique_words_val = list(set(flatten_reviews_val))
vocabulary_size_val = len(unique_words_val)
print('TRAIN dataset - vocabulary size: %s \nVAL dataset - vocabulary size: %s ' %
      (vocabulary_size, vocabulary_size_val))
...
print('Number of INCLUDED words: %s \nNumber of EXCLUDED words:%s' %
      (len(included_words), len(excluded_words)))
...
pl.xlabel('word occurrence')
pl.ylabel('probability density')
pl.title('[VAL] word occurences distribution')
pl.subplot(122)
sns.kdeplot(full_counts)
pl.xlabel('word occurrence')
pl.ylabel('probability density')
pl.title('[TRAIN] word occurences distribution')
pl.show()
