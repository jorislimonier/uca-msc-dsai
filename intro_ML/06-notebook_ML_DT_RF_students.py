...
X_train, X_test, y_train, y_test = train_test_split(
    pima.iloc[:, :-1], pima.Outcome, test_size=0.2, random_state=RAND_STATE)
...
sorted_features = sorted(
    zip(pima.columns[:-1, ], clf.feature_importances_), key=lambda couple: couple[1])
...
def plot_dataset(name, X_train, X_test, y_train, y_test):
    X0_train = np.array(
        [x for x, y in zip(X_train, y_train) if y == 0]).reshape((-1, 2))
    X1_train = np.array(
        [x for x, y in zip(X_train, y_train) if y == 1]).reshape((-1, 2))
    X0_test = np.array([x for x, y in zip(X_test, y_test)
                       if y == 0]).reshape((-1, 2))
    X1_test = np.array([x for x, y in zip(X_test, y_test)
                       if y == 1]).reshape((-1, 2))

    plt.figure(figsize=(15, 12))
    plt.scatter(X0_train[:, 0], X0_train[:, 1],
                c='b', alpha=0.1, label=["classe 0"])
    plt.scatter(X1_train[:, 0], X1_train[:, 1],
                c='r', alpha=0.1, label=["classe 1"])
...
for dataset in [True, False]:
    ...
    # Split dataset
    X_train, X_test, y_train, y_test = train_test_split(
        X, y, test_size=0.1, shuffle=True, stratify=y, random_state=RAND_STATE)
...
def plot_decision_boundary(classifiers, X_train, y_train, X_test, y_test):
    ...
    for i, (clf_name, clf) in enumerate(classifiers.items()):
        ...
        print("{:25s}: {:.2f} {:.2f} {:.2f} {:.2f} {: 3d}".format(
            clf_name[:25], ACC, P, R, F1, n_errors))
        ...
        if threshold <= Z.max():
            subplot.contourf(xx, yy, Z, levels=[threshold, Z.max()],
                             colors='orange')
        b = subplot.scatter(
            X0_test[:, 0], X0_test[:, 1], c='b', s=20, edgecolor='k')
        c = subplot.scatter(
            X1_test[:, 0], X1_test[:, 1], c='r', s=20, edgecolor='k')
        subplot.axis('tight')
        subplot.legend(
            [a.collections[0], b, c],
...
classifiers_ = {
    ...
    # Data Classifier Selection by Local Accuracy
    'DCS_LA': DCS_LA(base_estimators=classifier_list),
    # Dynamic Ensemble Selection by Local Accuracy
    'DES_LA': DES_LA(base_estimators=classifier_list),
}
